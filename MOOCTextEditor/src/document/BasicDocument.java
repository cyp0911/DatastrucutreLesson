...
public class BasicDocument extends Document 
{
	...
	public int getNumWords()
	{
		...
		// See the Module 2 support videos if you need help.
			...
			System.out.println("words" + tokens.size());
	    return tokens.size();
	}
	...
	public int getNumSentences()
	{
        ...
        // if you need help.
			...
			System.out.println("Sent" + tokens.size());
        return tokens.size();
	}
	...
	public int getNumSyllables()
	{
		...
		List<String> tokens = getTokens("[aAeEoOuUyYIi]+");
		List<String> lastchar = getTokens("[^b][eE]+[? .,$\n]");     // check if last char is velwo and not count
		List<String> lastIsE = getTokens("[^u][eE]+$");     // check if last char is velwo and not count
		List<String> senComma = getTokens("[c][e][,]");     // check if last char is velwo and not count

		
		int lastCharSwitch  = 0;
		if (lastIsE.size() > 0) {
			lastCharSwitch += 1;
		}
//		System.out.println("\n lastCharSwitch:" + lastCharSwitch);
//
//
//
//
...
//		
//		System.out.println("\n" + lastchar);
////		System.out.print("\n Num of Syllables(cyptest): " + tokens.size());
//		System.out.println(tokens);
		

		

	    int tokensize = tokens.size() - lastchar.size() - lastCharSwitch + senComma.size();
        return tokensize;
	}
